...
const FloatingButtons = () => {
  ...
  return (
    <div className={`floating-buttons-container ${!isHomePage ? 'other-page' : ''}`}>
      {actions.map(({ key, label, className, onClick, icon }) => (
        <div
          key={key}
          className={className}
          onClick={onClick}
        ...
        >
          {icon}
          {showTooltip[key] && <span className="floating-tooltip">{label}</span>}
        </div>
      ))}
    </div>
  );
